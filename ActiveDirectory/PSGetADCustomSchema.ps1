<#
PSGetADCustomSchema.ps1 - script outputs a text file with DNs of all custom schema. Uses LDIFDE cmmand.
...
Function PSGetADCustomSchema($strDomain)
{
	#function uses LDIFDE to get a list of custom schema (objects and attributes) in an AD domain.
	#result is a text file in the present working directory of DNs of custom schema.
	#1.$strDomain - domain to check - enter DN name and it will be converted to LDAP notation
	#object and attribute names pulled from URLs listed below
	
	$LDAPDomain = DNtoLDAP($strDomain)
	#export the LDAP schema to the pwd to a file (ldifde needs to export to a file)
	CMD /C "ldifde -f schema.ldif -d CN=Schema,CN=Configuration,$LDAPDomain"
	#import that file into a variable
	$ldifexport = Get-Content "schema.ldif" 
	<#regex to match the DN on attributes excluding:
	Default Windows Classes: http://msdn.microsoft.com/en-us/library/windows/desktop/ms680938(v=vs.85).aspx
	Default Attributes: http://msdn.microsoft.com/en-us/library/windows/desktop/ms675090(v=vs.85).aspx
	Evertyhing beginning with "ms-" (which will include ms-exch (exchange) amoung other things)
	
	Notes: There are nearly 2 thousand classes and attributes in the above URLs. To get these in the script I was able to copy/paste them from the website and use regular expressions to append a pipe at the end (regex for "or") and then paste that in this script.
	Yes - I used regular expressions to form a regular expression.
	#>
	
	$ldifexport -match "dn: CN=(?!
	ms-|
	MSMQ-|
	account|
	ACS-Policy|
